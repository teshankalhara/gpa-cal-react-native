...
import { GradeValue, Student } from '@/types';
...
export default function StudentCard({ student, gradeScale, onPress }: StudentCardProps) {
  const { colors } = useTheme();
  const scaleAnim = useRef(new Animated.Value(1)).current;
  const gpa = calculateCumulativeGPA(student.years, gradeScale);
  const gpaColor = getGPAColor(gpa);
  const totalSubjects = getTotalSubjects(student.years);
  const totalCredits = getTotalCredits(student.years);
        ...
        <View style={[styles.statsRow, { borderTopColor: colors.border }]}>
          <View style={styles.stat}>
            ...
            <Text style={[styles.statText, { color: colors.textSecondary }]}>
              {student.years.length} Year{student.years.length !== 1 ? 's' : ''}
            </Text>
          </View>
